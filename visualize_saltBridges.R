...
for (i in 1:5) {
  ...
  for (k in 1:length(localRes_List[[i]])) {
    saltbrFileName <- c(saltbrFileName, dir("3_WT/3_production/saltbridges_seokWT/", pattern = ".dat")[grep(paste0("\\b",localRes_List[[i]][k],"\\b"), dir("3_WT/3_production/saltbridges_seokWT/", pattern = ".dat"))])
    saltbrFileName <- c(saltbrFileName, dir(paste0(subgroup[i],"_",mutList[i],"/3_production/saltbridges_seok",mutList[i],"/"), pattern = ".dat")[grep(paste0("\\b",localRes_List[[i]][k],"\\b"), dir(paste0(subgroup[i],"_",mutList[i],"/3_production/saltbridges_seok",mutList[i],"/"), pattern = ".dat"))])
  }
  for (j in saltbrFileName) {
    if (j %in% dir("3_WT/3_production/saltbridges_seokWT/") &&
      ...
      pdf(file = paste0("_outputs/saltbridges/", mutList[i], "/WT_",mutList[i],"_", sub(".dat", "", j), ".pdf"), width = 8, height = 6)
      print(p)
      dev.off()
      
    } else if ((j %in% dir("3_WT/3_production/saltbridges_seokWT/")) &&
               !(j %in% dir(paste0(subgroup[i],"_",mutList[i],"/3_production/saltbridges_seok",mutList[i],"/")))) {
      ...
      p <- p + theme(axis.title.y = element_text(angle = 90, size = 24),
                     ...
                     legend.margin = margin(6, 6, 6, 6),
                     panel.border = element_rect(colour = "gray60", fill = NA, size = .5))
      pdf(file = paste0("_outputs/saltbridges/", mutList[i], "/WT_", sub(".dat", "", j), ".pdf"), width = 8, height = 6)
      print(p)
      dev.off()
      ...
      p <- p + theme(axis.title.y = element_text(angle = 90, size = 24),
                     ...
                     legend.margin = margin(6, 6, 6, 6),
                     panel.border = element_rect(colour = "gray60", fill = NA, size = .5))
      pdf(file = paste0("_outputs/saltbridges/", mutList[i], "/",mutList[i],"_", sub(".dat", "", j), ".pdf"), width = 8, height = 6)
      print(p)
      dev.off()
